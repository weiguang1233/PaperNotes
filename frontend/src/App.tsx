...
function SettingsView({ collections }: { collections: Collection[] }) {
  ...
  const chooseRoot = async () => { const chosen = window.prompt("请输入新的文献库目录路径", settings.data_root); if (!chosen?.trim()) return; const result = await putJson<{ restart_required: boolean }>("/api/v1/settings/data-root", { path: chosen.trim() }); setMessage(result.restart_required ? "新目录已保存，请重启 PaperNote 后生效。" : "数据目录未改变。"); };

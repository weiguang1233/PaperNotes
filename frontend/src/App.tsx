...
export default function App() {
  ...
  }, [view, filterStatuses, searchCriteria, noteStatuses, documentTypes, sortField, sortDirection]);

  const clearSearchAndFilters = () => {
    setSearchCriteria([{ id: criterionId.current++, field: "year", value: "" }]); setFilterStatuses([]);
  ...
  const hasActiveFilters = Boolean(activeCriteria.length || filterStatuses.length || noteStatuses.length || documentTypes.length || sortField !== "year" || sortDirection !== "desc");
    ...
    <main className="workspace">
      {(view === "library" || view === "review") && <>
        <header className="topbar">
          <div className="topbar-context"><strong>文献检索与管理</strong><span>请在下方组合检索区添加一个或多个条件</span></div>
        ...
        <section className="library-layout">
          <div className={`paper-pane ${selectedId ? "has-selection" : ""}`}>
            ...
            <div className="library-filterbar">
              ...
              <MultiFilter label="阅读状态" values={filterStatuses} options={[["unread", "未读"], ["reading", "在读"], ["read", "已读"], ["archived", "归档"]]} onChange={setFilterStatuses} />
            ...
            {hasActiveFilters && <div className="active-filter-summary"><strong>当前采用组合检索</strong><span>各检索行以及其他筛选栏目之间均需同时满足。</span>{activeCriteria.map((item, index) => <em key={item.id}>{index + 1}. {SEARCH_FIELD_OPTIONS.find(option => option[0] === item.field)?.[1]}：{item.value}</em>)}{filterStatuses.length > 0 && <em>阅读状态 {filterStatuses.length} 项</em>}{documentTypes.length > 0 && <em>文献类型 {documentTypes.length} 项</em>}{noteStatuses.length > 0 && <em>笔记状态 {noteStatuses.length} 项</em>}</div>}
